...
import { ArrowRight, ChevronRight, Eye, EyeOff, Lock, Mail, ShieldCheck, Sparkles, Zap } from "lucide-react";
...
export default function Login() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setApiError(error?.response?.data?.message || "Unable to sign in right now. Please verify the backend is running.");
    } finally {
      ...
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-7xl overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/60 shadow-[0_0_80px_rgba(37,99,235,0.18)] backdrop-blur-2xl">
        <motion.div initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.55, ease: "easeOut" }} className="flex w-full flex-col justify-between bg-gradient-to-br from-slate-950 via-slate-900 to-[#071224] p-6 sm:p-8 lg:w-[52%] lg:p-10">
          <div>
            ...
            <div className="mt-10 max-w-md sm:mt-12 lg:mt-16">
              <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }}>
                ...
                <p className="mt-3 text-base leading-7 text-slate-400 sm:text-lg">Sign in to access your AI Customer Resolution Workspace.</p>
              </motion.div>
              ...
              <motion.form initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.18, duration: 0.5 }} onSubmit={handleSubmit} className="mt-8 space-y-4">
                ...
                <div className="relative">
                  <InputField icon={Lock} type={showPassword ? "text" : "password"} name="password" placeholder="Password" value={form.password} onChange={handleChange} error={errors.password} autoComplete="current-password" />
                  <button type="button" onClick={() => setShowPassword((current) => !current)} className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full p-1 text-slate-400 transition hover:text-white" aria-label={showPassword ? "Hide password" : "Show password"}>
                ...
                <button type="button" className="flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3.5 text-sm font-medium text-slate-200 transition hover:border-blue-400/40 hover:bg-white/10">
          ...
          <div className="mt-8 flex items-center gap-4">
            <Link to="/register" className="text-sm text-blue-300 transition hover:text-blue-200">Create an account</Link>
        ...
        <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease: "easeOut", delay: 0.08 }} className="relative hidden w-[48%] items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_top_left,_rgba(37,99,235,0.28),_transparent_35%),linear-gradient(145deg,_rgba(15,23,42,0.95),_rgba(2,6,23,0.98))] p-8 lg:flex">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.55 }} className="relative w-full max-w-md rounded-[2rem] border border-white/10 bg-white/10 p-6 backdrop-blur-2xl">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {stats.map((stat, index) => (
                <motion.div key={stat.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.24 + index * 0.06, duration: 0.4 }} className="rounded-2xl border border-white/10 bg-slate-900/60 p-4">
            ...
            <div className="mt-6 rounded-[1.5rem] border border-white/10 bg-slate-950/70 p-5">
              <div className="flex items-center justify-between">
                ...
                <div className="rounded-full border border-violet-400/20 bg-violet-500/10 p-2 text-violet-300"><Zap className="h-4 w-4" /></div>
              </div>
              <div className="mt-5 space-y-3">
                {["Intent Capture", "Context Recall", "Resolution Draft", "Customer Reply"].map((step, index) => (
                  <div key={step} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-3 py-3">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-blue-600 to-violet-600 text-sm font-semibold text-white">{index + 1}</div>
                    ...
                    <motion.div animate={{ scale: [1, 1.08, 1] }} transition={{ repeat: Infinity, duration: 2.2, delay: index * 0.2 }}>
                      <div className="h-5 w-5 rounded-full border border-emerald-400/30 bg-emerald-400/20" />
                    </motion.div>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-6 rounded-[1.5rem] border border-cyan-400/20 bg-cyan-500/10 p-4 text-sm text-cyan-100">
              <div className="flex items-center gap-2"><Sparkles className="h-4 w-4" /><span className="font-medium">Adaptive orchestration is actively resolving tickets.</span></div>
            </div>
          </motion.div>
        </motion.div>
      </div>
